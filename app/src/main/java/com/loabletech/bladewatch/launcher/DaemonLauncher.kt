...
class DaemonLauncher(
    ...
    private fun writeCamDaemonScript(
        ...
        val scriptLines = listOf(
            ...
            "    exit 0",
            "  fi",
            "  # Lock file pre-flight check: if the lock file contains a live PID,",
            "  # another instance is already running — do NOT spawn a second one.",
            "  # This is a belt-and-suspenders guard alongside the daemon's own",
            "  # acquireSingletonLock() and port-availability check.",
            "  if [ -f \"\$LOCK_FILE\" ]; then",
            "    LOCKED_PID=\$(cat \"\$LOCK_FILE\" 2>/dev/null)",
            "    if [ -n \"\$LOCKED_PID\" ] && [ -d \"/proc/\$LOCKED_PID\" ] 2>/dev/null; then",
            "      echo \"[\$(date)] Another CameraDaemon instance (PID \$LOCKED_PID) holds the lock. Not starting.\" >> \"\$LOG_FILE\"",
            "      exit 0",
            "    fi",
